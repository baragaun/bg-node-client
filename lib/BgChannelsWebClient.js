...
export class BgChannelsWebClient {
    ...
    createMockChannel(attributes, userCount, messageCount, users, messages) {
        return factories.channel(attributes, userCount, messageCount, users, messages);
    }
    createMockUser(attributes) {
        return factories.user(attributes);
    }
    /**
    ...
    async findChannels(filter, match, skip, limit) {
        const result = await findChannelsFunc(filter, match, skip, limit);
        return result;
    }
    /**
...
//# sourceMappingURL=BgChannelsWebClient.js.map