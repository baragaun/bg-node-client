...
export class BgNodeClient {
    ...
    factories = factories;
    libSignalStores = db.libSignalStores;
    /**
    ...
    async findMyUser() {
        ...
        if (useCached) {
            const result = await findByIdFunc(this._myUserId, ModelType.MyUser);
            if (result.error) {
                return null;
            }
            return result.object;
    ...
    async insertOne(object) {
        const result = await insertOneFunc(object);
        if (result.error) {
            return null;
        }
        return result.object;
    }
    async signInUser(ident, identType, password) {
        ...
        if (!result.error) {
            ...
            if (result.object.userId) {
                ...
                return insertOneFunc(myUser);
            }
        }
        return result;
    }
    async signUpUser(userHandle, email, password) {
