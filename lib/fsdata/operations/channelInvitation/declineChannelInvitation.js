...
const declineChannelInvitation = async (channelInvitationId, reasonTextId, queryOptions = defaultQueryOptionsForMutations) => {
    try {
        ...
        logger.debug('fsdata.declineChannelInvitation received response.', { response: JSON.stringify(response) });
        if (Array.isArray(response.errors) && response.errors.length > 0) {
            logger.error('fsdata.declineChannelInvitation: errors received.', { errorCode: response.errors['0']?.extensions?.code, errors: JSON.stringify(response.errors) });
