...
const findChannelInvitationsForUser = async (userId, onlyUnseen, onlyPending, direction, options) => {
    try {
        ...
        logger.debug('fsdata.findChannelInvitationsForUser received response.', { response: JSON.stringify(response) });
        if (response.error) {
            logger.error('fsdata.findChannelInvitationsForUser: errors received.', { errorCode: response.error?.extensions?.code, errors: JSON.stringify(response.error) });
