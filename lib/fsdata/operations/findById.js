...
const findById = async (id, modelType, selections, options) => {
    try {
        ...
        logger.debug('fsdata.findById: response received.', { response: JSON.stringify(response) });
        ...
        return { object: modelFactory(response.data[fieldDef.findByIdField], modelType) };
    }
    catch (error) {
