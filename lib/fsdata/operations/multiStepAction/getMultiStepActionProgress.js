...
const getMultiStepActionProgress = async (actionId, confirmToken) => {
    ...
    try {
        ...
        if (response.getMultiStepActionProgress.notificationResult !== previousAction.notificationResult) {
            if (response.getMultiStepActionProgress.notificationResult ===
        ...
        if (previousAction.result === MultiStepActionResult.Unset &&
