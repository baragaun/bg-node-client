...
const verifyMultiStepActionToken = async (actionId, confirmToken, newPassword) => {
    try {
        ...
        logger.debug('fsdata.verifyMultiStepActionToken: received response.', { response: JSON.stringify(response) });
        if (response.error) {
            logger.error('fsdata.verifyMultiStepActionToken: errors received.', { errorCode: response.error?.extensions?.code, errors: JSON.stringify(response.error) });
