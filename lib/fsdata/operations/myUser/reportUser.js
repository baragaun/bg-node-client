...
const reportUser = async (userId, reasonTextId, messageText, _queryOptions = defaultQueryOptionsForMutations) => {
    try {
        ...
        logger.debug('fsdata.reportUser: response received.', { response: JSON.stringify(response) });
        if (Array.isArray(response.errors) && response.errors.length > 0) {
            logger.error('fsdata.reportUser: errors received.', { errorCode: response.errors['0']?.extensions?.code, errors: JSON.stringify(response.errors) });
