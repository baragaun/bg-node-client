...
const startMySessionV2 = async (pushNotificationToken, returnContentStatus) => {
    try {
        ...
        logger.debug('fsdata.startMySessionV2: response received.', { response: JSON.stringify(response) });
        if (Array.isArray(response.errors) && response.errors.length > 0) {
            logger.error('fsdata.startMySessionV2: errors received.', { errorCode: response.errors['0']?.extensions?.code, errors: JSON.stringify(response.errors) });
