...
const unblockUserForMe = async (userId, queryOptions = defaultQueryOptionsForMutations) => {
    try {
        ...
        logger.debug('fsdata.unblockUserForMe: response received.', { response: JSON.stringify(response) });
        if (Array.isArray(response.errors) && response.errors.length > 0) {
            logger.error('fsdata.unblockUserForMe: errors received.', { errorCode: response.errors['0']?.extensions?.code, errors: JSON.stringify(response.errors) });
