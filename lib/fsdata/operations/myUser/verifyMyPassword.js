...
const verifyMyPassword = async (password) => {
    try {
        ...
        logger.debug('fsdata.verifyMyPassword: response received.', { response: JSON.stringify(response) });
        if (response.error) {
            logger.error('fsdata.verifyMyPassword: errors received.', { errorCode: response.error?.extensions?.code, errors: JSON.stringify(response.error) });
