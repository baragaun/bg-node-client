...
const clearMyShoppingCart = async () => {
    try {
        ...
        logger.debug('fsdata.clearMyShoppingCart response:', { response: JSON.stringify(response) });
        if (response.error) {
            logger.error('fsdata.clearMyShoppingCart: errors received.', { errorCode: response.error?.extensions?.code, errors: JSON.stringify(response.error) });
