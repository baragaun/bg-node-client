...
const update = async (changes, modelType, queryOptions = defaultQueryOptionsForMutations) => {
    try {
        ...
        if (Array.isArray(updateResponse.errors) && updateResponse.errors.length > 0) {
            logger.error('fsdata.createChannelInvitation: errors received', { errorCode: updateResponse.errors['0'].extensions.code, errors: JSON.stringify(updateResponse.errors) });
