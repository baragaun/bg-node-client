...
const findMyWallet = async () => {
    try {
        ...
        if (response.error) {
            logger.error('fsdata.findMyWallet: errors received.', { errorCode: response.error?.extensions?.code,
                errors: JSON.stringify(response.error) });
            return { error: response.error };
        }
        if (!response.data || !response.data.findMyWallet) {
            logger.error('fsdata.findMyWallet: errors', { errors: response.error, headers: helpers.headers() });
            return { error: response.error };
        }
        return {
