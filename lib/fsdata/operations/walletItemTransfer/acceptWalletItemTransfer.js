...
const acceptWalletItemTransfer = async (transferSlug, transferSecret) => {
    try {
        ...
        logger.debug('fsdata.acceptWalletItemTransfer response:', { transferSlug, response: JSON.stringify(response) });
        ...
        if (!walletItemId) {
            logger.error('fsdata.acceptWalletItemTransfer: failed to extract walletItemId from serviceRequest.', { serviceRequestId: serviceRequest.id, transferSlug, pollingResponse });
