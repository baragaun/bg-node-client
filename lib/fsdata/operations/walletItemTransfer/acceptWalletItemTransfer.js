...
const acceptWalletItemTransfer = async (transferSlug, transferSecret) => {
    try {
        ...
        if (response.error) {
            logger.error('fsdata.acceptWalletItemTransfer: errors received.', { transferSlug, errorCode: response.error?.extensions?.code, errors: JSON.stringify(response.error) });
