...
const declineWalletItemTransfer = async (transferSlug) => {
    try {
        ...
        if (response.error) {
            logger.error('fsdata.declineWalletItemTransfer: errors received', { transferSlug, errorCode: response.error?.extensions?.code, errors: JSON.stringify(response.error) });
        ...
        if (serviceRequest.result === ServiceRequestResult.error) {
            logger.error('fsdata.declineWalletItemTransfer: processes failed.', { transferSlug, serviceRequest });
            return { error: pollingResponse.error, serviceRequest };
        }
        if (!serviceRequest ||
        ...
        if (!walletItemId) {
            logger.error('fsdata.declineWalletItemTransfer: failed to extract walletItemId from serviceRequest.', { transferSlug, serviceRequestId: serviceRequest.id, pollingResponse });
            return { error: ErrorCode.SystemError, serviceRequest };
        }
