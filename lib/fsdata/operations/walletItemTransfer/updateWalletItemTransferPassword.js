...
const updateWalletItemTransferPassword = async (transferSlug, transferSecret, password) => {
    try {
        ...
        if (response.error) {
            logger.error('fsdata.updateWalletItemTransferPassword: errors received.', { transferSlug, errorCode: response.error?.extensions?.code, errors: JSON.stringify(response.error) });
