...
const updateWalletItemTransferPassword = async (transferSlug, transferSecret, password) => {
    try {
        ...
        const response = await client.mutation.updateWalletItemTransferPassword({
            $: args,
            ...modelFields.serviceRequest,
        });
        logger.debug('fsdata.updateWalletItemTransferPassword response received.', { transferSlug, response: JSON.stringify(response) });
        if (Array.isArray(response.errors) && response.errors.length > 0) {
            logger.error('fsdata.updateWalletItemTransferPassword: errors received.', { transferSlug, errorCode: response.errors['0']?.extensions?.code, errors: JSON.stringify(response.errors) });
        ...
        logger.debug('fsdata.updateWalletItemTransferPassword: finished.', { transferSlug, pollingResponse });
        if (pollingResponse.error) {
            logger.error('fsdata.updateWalletItemTransferPassword: polling failed', { transferSlug, error: pollingResponse.error });
            return { error: pollingResponse.error, serviceRequest };
        }
        if (serviceRequest.result === ServiceRequestResult.error) {
            logger.error('fsdata.updateWalletItemTransferPassword: processes failed.', { transferSlug, serviceRequest });
            return { error: pollingResponse.error, serviceRequest };
        }
        return { serviceRequest };
    }
    catch (error) {
        logger.error('fsdata.updateWalletItemTransferPassword: error.', { transferSlug, error, headers: helpers.headers() });
