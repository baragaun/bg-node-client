...
const updateWalletItemTransferShowOnlineFlag = async (transferSlug, transferSecret, showOnline) => {
    try {
        ...
        if (response.error) {
            logger.error('fsdata.updateWalletItemTransferShowOnlineFlag: errors received', { transferSlug, errorCode: response.error?.extensions?.code, errors: JSON.stringify(response.error) });
