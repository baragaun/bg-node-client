...
const updateWalletItemTransferShowOnlineFlag = async (transferSlug, transferSecret, showOnline) => {
    try {
        ...
        if (!walletItemId) {
            logger.error('fsdata.updateWalletItemTransferShowOnlineFlag: failed to extract walletItemId from serviceRequest.', { transferSlug, serviceRequestId: serviceRequest.id, pollingResponse });
            return { error: ErrorCode.SystemError, serviceRequest };
        }
        const findResult = await findById(walletItemId, ModelType.WalletItem);
        if (findResult.error) {
            logger.error('fsdata.updateWalletItemTransferShowOnlineFlag: error loading wallet item transfer', { transferSlug, error: findResult.error, walletItemId });
            return { error: findResult.error, serviceRequest };
        }
        if (!findResult.object) {
            logger.error('fsdata.updateWalletItemTransferShowOnlineFlag: wallet item transfer not found', { transferSlug, walletItemId });
