...
const libData = {
    ...
    setObjectInCache: (modelType, id, object) => {
        const key = `${modelType}-${id}`;
        objectCache.set(key, object);
    },
    setObjectListInCache: (modelType, object) => {
        objectCache.set(modelType, object);
    },
    setObjectInCachedList: (modelType, id, object) => {
    ...
    getObjectFromCachedList: (modelType, id) => {
        const list = objectCache.get(modelType);
        if (!Array.isArray(list)) {
            return null;
        }
    ...
    getObjectFromCache: (modelType, id) => {
        const key = `${modelType}-${id}`;
        return objectCache.get(key) || null;
    },
    getObjectListFromCache: (modelType) => {
        return objectCache.get(modelType) || null;
    },
    deleteObjectFromCache: (modelType, id) => {
        const key = `${modelType}-${id}`;
        objectCache.delete(key);
    },
    deleteObjectListFromCache: (modelType) => {
