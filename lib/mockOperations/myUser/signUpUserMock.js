...
const signUpUserMock = async (input) => {
    ...
    try {
        ...
        const insertResponse = await db.insert(myUser, ModelType.MyUser);
        if (insertResponse.error || !insertResponse.object) {
            logger.error('signUpUserMock: failed to insert mock user into db', { myUser, error: insertResponse.error });
            return { error: 'system-error' };
        }
        const signInSignUpResponse = {
            userAuthResponse: {
                userId: myUserId,
                authToken,
                authTokenExpiresAt,
            ...
            if (listener.topic === BgListenerTopic.myUser &&
                ...
                if (listenerResponse && typeof listenerResponse.then === 'function') {
                    listenerResponse.catch((error) => {
                        logger.error('signUpUserMock: listener onSignedIn failed.', { error });
                    });
                }
            }
        }
...
//# sourceMappingURL=signUpUserMock.js.map