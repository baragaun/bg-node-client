...
export const processChannelEvent = async (payload) => {
    const processor = processors[payload.reason];
    if (typeof processor !== 'function') {
        logger.debug('processChannelEvent: No processor for this Channel event reason.', { reason: payload.reason });
        return;
    }
    return processor(payload);
};
//# sourceMappingURL=processChannelEvent.js.map