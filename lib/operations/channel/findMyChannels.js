...
const findMyChannels = async (filter, match, options, scope, queryOptions = defaultQueryOptions) => {
    try {
        ...
        const result = await fsdata.channel.findMyChannels(filter, match, options, scope);
        if (result.error) {
            ...
            for (const channelListItem of result.objects) {
                ...
                if (Array.isArray(channelListItem.participants)) {
                    for (const participant of channelListItem.participants) {
                        await db.upsert({ channelId: channel.id, ...participant }, ModelType.ChannelParticipant);
                    }
                }
