...
const findMyChannelsV2 = async (participantLimit, addLatestMessage, options, queryOptions = defaultQueryOptions) => {
    try {
        if (!libData.isInitialized()) {
            logger.error('findMyChannelsV2: unavailable');
            return { error: 'unavailable' };
        }
        if (!libData.clientInfoStore().isSignedIn) {
            logger.error('findMyChannelsV2: unauthorized');
            return { error: 'unauthorized' };
        }
        const myUserId = libData.clientInfoStore().myUserId;
        if (!myUserId) {
            logger.error('findMyChannelsV2: myUserId not set');
            return { error: 'unauthorized' };
        }
        ...
        if (queryOptions.cachePolicy === CachePolicy.cacheFirst || !allowNetwork) {
            const localQueryForChannels = buildQuery(ModelType.Channel, undefined, undefined, { userIds: { $in: [myUserId] } }, options);
        ...
        const result = await fsdata.channel.findMyChannelsV2(participantLimit, addLatestMessage, options);
        if (result.error) {
            logger.error('findMyChannelsV2: error from fsdata', { error: result.error });
            ...
            for (const channelListItem of result.objects) {
                ...
                if (channelListItem.latestMessage) {
                    try {
                        ...
                        logger.error('findMyChannelsV2: error inserting latest message', {
                            error: error.message,
