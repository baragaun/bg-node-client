import { CachePolicy, MultiStepActionEventType, MultiStepActionResult, MultiStepActionSendNotificationResult, MultiStepActionType, } from '../../enums.js';
...
const getMultiStepActionProgress = async (actionId, confirmToken, queryOptions) => {
    logger.debug('BgNodeClient.operations.multiStepAction.getMultiStepActionProgress called.', { actionId, confirmToken, queryOptions });
    const clientInfo = clientInfoStore.get();
    const result = {};
    try {
        const actionProgress = await fsdata.multiStepAction.getMultiStepActionProgress(actionId, confirmToken);
        if (!actionProgress) {
            logger.error('BgNodeClient.operations.multiStepAction.getMultiStepActionProgress: received error.', { actionId, confirmToken });
            result.error = 'not-found';
            return result;
        }
        let run = libData.multiStepActionRun(actionProgress.actionId);
        ...
        logger.debug('BgNodeClient.operations.multiStepAction.getMultiStepActionProgress: run.', { actionProgress, run });
        // Has the notification been sent?
        if (!previousProgress?.notificationResult &&
            actionProgress.notificationResult) {
            if (actionProgress.notificationResult ===
        ...
        if (queryOptions.polling.enabled &&
        ...
        logger.error('BgNodeClient.operations.multiStepAction.getMultiStepActionProgress: failed.', { error });
        return null;
    }
};
