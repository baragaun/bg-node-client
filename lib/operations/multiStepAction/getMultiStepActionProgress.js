...
const getMultiStepActionProgress = async (actionId, confirmToken, queryOptions) => {
    ...
    try {
        ...
        console.log('BgNodeClient.operations.multiStepAction.getMultiStepActionProgress: received response.', { actionProgress });
        if (!actionProgress) {
            console.error('BgNodeClient.operations.multiStepAction.getMultiStepActionProgress: action not found.');
            result.error = 'not-found';
            return result;
        }
        let run = data.multiStepActionRun(actionProgress.actionId);
