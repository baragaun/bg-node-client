...
const signInUser = async (input) => {
    try {
        ...
        if (signInUserResponse.error || !userAuthResponse) {
            logger.error('signInUser: failed to sign in user', { input, error: signInUserResponse.error });
        ...
        if (findMyUserResult.error || !findMyUserResult.object) {
            logger.error('signInUser: failed to find my user after sign in', { findMyUserResult, userAuthResponse });
            return { error: 'system-error' };
        }
            ...
            if (listener.topic === BgListenerTopic.myUser &&
                ...
                if (listenerResponse && typeof listenerResponse.then === 'function') {
                    listenerResponse.catch((error) => {
                        logger.error('signInUser: listener onSignedIn failed.', { error });
                    });
                }
            }
        }
