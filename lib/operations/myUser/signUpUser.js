...
const signUpUser = async (input) => {
    try {
        ...
        if (findMyUserResult.error || !findMyUserResult.object) {
            logger.error('signUpUser: failed to find my user after sign up', { findMyUserResult, userAuthResponse });
            return { error: 'system-error' };
        }
            ...
            if (listener.topic === BgListenerTopic.myUser &&
                ...
                if (listenerResponse && typeof listenerResponse.then === 'function') {
                    listenerResponse.catch((error) => {
                        logger.error('signUpUser: listener onSignedIn failed.', { error });
                    });
                }
            }
        }
