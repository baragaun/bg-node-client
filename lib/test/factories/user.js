...
const userFactory = Factory.define('User', User)
    ...
    .sequence('email', () => uniqueEmail(libData.config()?.testEmailPrefix || 'test', libData.config()?.testEmailDomain || 'test.com'))
    ...
    ? Array.from({ length: chance.integer({ min: 0, max: 3 }) }).map((_) => new LabeledStringValue({
        label: chance.word(),
        value: chance.url(),
    }))
    : undefined)
    .attr('userHandle', () => uniqueUserHandle())
    .attr('adminNotes', () => JSON.stringify({ password: chance.string({ length: 8 }) }));
