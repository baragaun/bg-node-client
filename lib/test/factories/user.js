...
const userFactory = Factory.define('User', User)
    ...
    .attr('fallbackUiLanguageTextId', () => chance.bool({ likelihood: 80 }) ? UiLanguage.en : chance.pickone(Object.values(UiLanguage)))
    .attr('firstName', () => chance.first())
    .attr('lastName', () => chance.last())
    .attr('genderTextId', () => chance.bool({ likelihood: 30 }) ? chance.pickone([undefined, '', '-', 'f', 'm', 'x']) : undefined)
    ...
    .attr('preferredLanguageTextId', () => chance.bool({ likelihood: 9 }) ? chance.pickone(Object.values(UiLanguage)) : undefined)
    .attr('regionOfResidence', () => (chance.bool({ likelihood: 10 }) ? chance.state() : undefined))
    .attr('selectedUiLanguageTextId', () => chance.bool({ likelihood: 30 }) ? UiLanguage.en : chance.pickone(Object.values(UiLanguage)))
    .attr('source', () => (chance.bool({ likelihood: 10 }) ? chance.word() : undefined))
    .attr('spokenLanguagesTextIds', () => chance.bool({ likelihood: 10 })
    ? chance.pickset(Object.values(UiLanguage), chance.integer({ min: 0, max: 2 }))
    : undefined)
    .attr('trustLevel', () => chance.bool({ likelihood: 0.1 })
    ? chance.integer({ min: -10, max: -1 }) // 0.1% are marked as bad actors
    : chance.integer({ min: 1, max: 1000 }))
    .attr('websites', () => chance.bool({ likelihood: 40 })
    ? Array.from({ length: chance.integer({ min: 0, max: 3 }) }).map((_) => new LabeledStringValue({ label: chance.word(), value: chance.url() }))
    : undefined)
    .attr('userHandle', () => chance.word());
userFactory.create = (props, options, count) => create(props, ModelType.User, options, count);
...
//   return object;
// };
userFactory.save = async (user) => save(user);
userFactory.delete = async (user) => {
