...
const userFactory = Factory.define('User', User)
    .attr('avatarUrl', () => chance.bool({ likelihood: 10 })
    ? chance.url()
    : undefined)
    .attr('cityOfResidence', () => chance.bool({ likelihood: 10 })
    ? chance.city()
    : undefined)
    ...
    ? chance.phone()
    : undefined)
    .attr('postalCode', () => chance.bool({ likelihood: 90 })
    ? chance.postcode()
    : undefined)
    .attr('preferredLanguageTextId', () => chance.bool({ likelihood: 9 })
    ? chance.pickone(Object.values(UiLanguage))
    : undefined)
    .attr('regionOfResidence', () => chance.bool({ likelihood: 10 })
    ? chance.state()
    : undefined)
    .attr('selectedUiLanguageTextId', () => chance.bool({ likelihood: 30 })
    ? UiLanguage.en
    : chance.pickone(Object.values(UiLanguage)))
    .attr('source', () => chance.bool({ likelihood: 10 })
    ? chance.word()
    : undefined)
    .attr('spokenLanguagesTextIds', () => chance.bool({ likelihood: 10 })
    ? chance.pickset(Object.values(UiLanguage), chance.integer({ min: 0, max: 2 }))
    : undefined)
    .attr('trustLevel', () => chance.bool({ likelihood: .1 })
