...
export const createChannelSpecHelper = async (props, messageCount, client) => {
    ...
    if (messageCount && messageCount > 0) {
        ...
        // Check if NATS client is available for message verification
        ...
        if (natsClient) {
            try {
                ...
                streamName = await findStreamNameBySubject(natsSubject);
                consumerName = `first-spark-dev-channel-${channel.id}-messages`;
                ...
                js = await natsClient.getJetStreamClient();
                jsm = await natsClient.getJetStreamManager();
                await jsm.consumers.add(streamName, consumerConfig);
            }
            catch (error) {
                logger.debug('NATS setup failed, skipping NATS verification', { error });
                js = null;
                jsm = null;
                streamName = null;
                consumerName = null;
        ...
        // Verify NATS messages if NATS is available
        if (js && jsm && streamName && consumerName) {
            try {
                ...
                logger.debug('NATS message verification failed', { error });
            }
        }
    }
    return channel;
