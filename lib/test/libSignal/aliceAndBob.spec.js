...
describe('libSignal', () => {
    ...
    test('should exchange end-to-end encrypted messages', async () => {
        ...
        const alice = { id: crypto.randomUUID().replaceAll('-', ''), firstName: 'Alice' };
        const bob = { id: crypto.randomUUID().replaceAll('-', ''), firstName: 'Bob' };
        ...
        const bobsPreKeyBundle = useX3DH
        ...
        const bobsPreKeyBundleAsReceived = jsonToPreKeyBundle(bobsPreKeyBundleSerialized);
        ...
        await SignalClient.processPreKeyBundle(bobsPreKeyBundleAsReceived, bobAddress, aliceStores.session, aliceStores.identity);
        // Alice encrypts a chat message text:
        let encryptedMessage1;
        try {
            encryptedMessage1 = await SignalClient.signalEncrypt(channelMessage1.buffer, bobAddress, aliceStores.session, aliceStores.identity);
        }
        catch (error) {
            console.error('Error encrypting message:', error);
        }
        expect(encryptedMessage1.type()).toBe(SignalClient.CiphertextMessageType.PreKey);
        const cipherTextR1 = SignalClient.PreKeySignalMessage.deserialize(encryptedMessage1.serialize());
        ...
        const bDPlaintext = await SignalClient.signalDecryptPreKey(cipherTextR1AsReceived, aliceAddress, bobStores.session, bobStores.identity, bobStores.prekey, bobStores.signed, bobStores.kyber);
        ...
        const encryptedMessage2 = await SignalClient.signalEncrypt(channelMessage2.buffer, aliceAddress, bobStores.session, bobStores.identity);
        ...
        const aDPlaintext = await SignalClient.signalDecrypt(bCiphertextR, bobAddress, aliceStores.session, aliceStores.identity);
...
//# sourceMappingURL=aliceAndBob.spec.js.map