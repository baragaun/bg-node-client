...
describe.runIf(isFeatureEnabled('channels'))('operations.channel.findMyChannels', () => {
    ...
    test('returns the channels for the current user', async () => {
        ...
        const channels = await Promise.all(Array.from({ length: count }, () => createChannelSpecHelper({ otherUserId: otherUser.id }, 0, client)));
        const channelIds = channels.map((c) => c.id).sort();
        // Fetching channels from the network:
        const queryResultFromNetwork = await client.operations.channel.findMyChannels(undefined, undefined, undefined, undefined, { cachePolicy: CachePolicy.network });
        ...
        const queryResultFromLocal = await client.operations.channel.findMyChannels(undefined, undefined, undefined, undefined, { cachePolicy: CachePolicy.cache });
