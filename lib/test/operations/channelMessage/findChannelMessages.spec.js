...
import { createMultipleChannelSpecHelper, } from '../../helpers/channel/createMultipleChannels.specHelper.js';
...
describe('operations.channelMessage.findChannelMessages', () => {
    ...
    test('finds channel messages', async () => {
        ...
        const queryResultFromNetwork = await client.operations.channelMessage.findChannelMessages(undefined, { channelId: channels[0].id }, undefined, { sort: [{ field: 'createdAt', direction: SortDirection.desc }] }, { cachePolicy: CachePolicy.network });
        ...
        for (let i = 0; i < channelMessagesFromNetwork.length; i++) {
            ...
            if (i > 0) {
                expect(new Date(channelMessage.createdAt).getTime()).toBeLessThan(new Date(channelMessagesFromNetwork[i - 1].createdAt).getTime());
            }
        }
        // Fetching first batch of a paginated sequence:
        const limit0 = 6;
        const queryResultFromNetworkBatch0 = await client.operations.channelMessage.findChannelMessages(undefined, { channelId: channels[0].id }, undefined, { sort: [{ field: 'createdAt', direction: SortDirection.desc }], skip: 0, limit: limit0 }, { cachePolicy: CachePolicy.network });
        ...
        const queryResultFromNetworkBatch1 = await client.operations.channelMessage.findChannelMessages(undefined, { channelId: channels[0].id }, undefined, { sort: [{ field: 'createdAt', direction: SortDirection.desc }], skip: limit0, limit: 50 }, // We'll request 50, but will only get 4
        ...
        const queryResultFromLocal = await client.operations.channelMessage.findChannelMessages(undefined, { channelId: channels[0].id }, undefined, undefined, { cachePolicy: CachePolicy.cache });
        ...
        await Promise.all(otherChannels.map((channel) => deleteChannelSpecHelper(channel.id, client)));
    });
}, { timeout: 20000 });
//# sourceMappingURL=findChannelMessages.spec.js.map