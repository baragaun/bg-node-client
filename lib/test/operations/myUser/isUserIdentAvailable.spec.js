...
describe('operations.myUser.isUserIdentAvailable', () => {
    ...
    test('should return false for an email that is already taken', async () => {
        ...
        const email = chance.email();
        let resultEmail = await client.operations.myUser.isUserIdentAvailable(email, UserIdentType.email);
        let resultUserHandle = await client.operations.myUser.isUserIdentAvailable(userHandle, UserIdentType.userHandle);
        ...
        resultUserHandle = await client.operations.myUser.isUserIdentAvailable(userHandle, UserIdentType.userHandle);
