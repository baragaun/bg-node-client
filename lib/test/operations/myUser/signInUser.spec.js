...
describe('signInUser', () => {
    test('should sign in a user with valid input', async () => {
        ...
        const { object: signInUserAuthResponse } = await client.operations.myUser.signInUser(email, UserIdentType.Email, password);
        console.log('Sign In User', signInUserAuthResponse);
        const { object: reloadedUser, error } = await findById(signInUserAuthResponse.userId, ModelType.MyUser);
