...
describe.runIf(isFeatureEnabled('marketplace'))('operations.walletItemTransfer.verifyWalletItemTransfer', () => {
    let client;
    let myUser;
    beforeAll(async () => {
    ...
    test('should verify wallet item transfer using transferSecret and walletItemId', async () => {
        ...
        expect(walletItemTransfer.walletItemId).toBe(walletItem.id);
        const transferSecret = walletItemTransfer.transferSecret;
        const walletItemId = walletItemTransfer.walletItemId;
        expect(transferSecret).toBeDefined();
        expect(typeof transferSecret).toBe('string');
        expect(transferSecret.length).toBeGreaterThan(0);
        const verifyResult = await client.operations.walletItemTransfer.verifyWalletItemTransfer(transferSecret, walletItemId);
        expect(verifyResult.error).toBeUndefined();
        expect(verifyResult.object).toBeDefined();
        expect(verifyResult.object?.id).toBe(walletItemId);
    });
}, 10000);
//# sourceMappingURL=verifyWalletItemTransfer.spec.js.map