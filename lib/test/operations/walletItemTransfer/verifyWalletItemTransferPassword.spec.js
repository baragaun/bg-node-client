...
describe('operations.walletItemTransfer.verifyWalletItemTransferPassword', () => {
    ...
    test('should verify correct password successfully', async () => {
        ...
        const updateResult = await updateWalletItemTransferPassword(transferSlug, transferSecret, testPassword);
        expect(updateResult.error).toBeUndefined();
        // Now verify the password
        const verifyResult = await verifyWalletItemTransferPassword(transferSlug, testPassword);
    ...
    test('should return false for incorrect password', async () => {
        ...
        const result = await verifyWalletItemTransferPassword(transferSlug, incorrectPassword);
        expect(result.error).toBeUndefined();
        expect(result.object).toBe(false);
    }, 10000);
    test('should handle empty password verification', async () => {
        const emptyPassword = '';
        const result = await verifyWalletItemTransferPassword(transferSlug, emptyPassword);
        expect(result.error).toBeUndefined();
        expect(result.object).toBe(false);
    }, 10000);
    test('should be case sensitive for password verification', async () => {
        ...
        const updateResult = await updateWalletItemTransferPassword(transferSlug, transferSecret, mixedCasePassword);
        expect(updateResult.error).toBeUndefined();
        // Try to verify with different case
        const wrongCasePassword = 'testpassword123';
        const verifyResult = await verifyWalletItemTransferPassword(transferSlug, wrongCasePassword);
        expect(verifyResult.error).toBeUndefined();
        expect(verifyResult.object).toBe(false);
    }, 20000);
});
//# sourceMappingURL=verifyWalletItemTransferPassword.spec.js.map