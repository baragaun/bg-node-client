...
const declineWalletItemTransfer = async (
  ...
  try {
    ...
    if (serviceRequest.result === ServiceRequestResult.error) {
      logger.error('fsdata.declineWalletItemTransfer: processes failed.',
    ...
    if (!walletItemId) {
      logger.error('fsdata.declineWalletItemTransfer: failed to extract walletItemId from serviceRequest.',
