...
export const createChannelMessageSpecHelper = async (
  props: Partial<ChannelMessage> | undefined,
  client: BgNodeClient,
    ...
    const { object: messageFromLocal, error: errorFromCache } = await findById<ChannelMessage>(
      messageFromNetwork.id,
    ...
    if (!client.isInMockMode) {
        ...
        const { object: reloadedMessageFromNetwork, error: errorFromNetwork } = await findById<ChannelMessage>(
          messageFromNetwork.id,
          ModelType.ChannelMessage,
