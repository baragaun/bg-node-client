...
export const createShoppingCartItemSpecHelper = async (
  props: Partial<ShoppingCartItem> | undefined,
  client: BgNodeClient,
): Promise<ShoppingCartItem | null> => {
  logger.debug('BgServiceApiCheck.createShoppingCartItem: calling API/createShoppingCartItem',
    { props });
  ...
  if (!client.isInMockMode) {
    ...
    const { object: shoppingCartItemFromNetwork, error: errorFromNetwork } = await findById<ShoppingCartItem>(
      shoppingCartItem.id,
