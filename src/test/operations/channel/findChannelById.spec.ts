...
describe('operations.channel.findChannelById', () => {
  ...
  test('returns a channel', async () => {
    ...
    const queryResultFromNetwork = await client.operations.channel.findChannelById(
      channel.id,
      {
    ...
    const queryResultFromLocal = await client.operations.channel.findChannelById(
      channel.id,
      {
        includeMessages: true,
        includeParticipants: true,
